...
function scroll() {
    ...
    svg.style.transform = `scale(${0.95 + textP * 0.45})`;

    for (let j = 0; j < visiblerings; j++) {
        const idx = k - j;
        ts[j].textContent = (idx >= 0 && idx < lines.length) ? lines[idx] : "";

        const t = j / (visiblerings - 1);
        const opacity = (j === visiblerings - 1) ? 0 : (1 - t * 0.25);
    }

    group.setAttribute("transform", `rotate(${(textP - 0.5) * 3})`);
    ...
    if (p < firebegin) {
        ...
        const e = movein(fireP);
        const y = (1 - e) * 110;
        firepng.style.transform = `translateY(${y}vh)`;
    }
    ...
    if (p < blackscreenup) {
        ...
        const e = movein(blackP);
        const y = (1 - e) * 100;
        blackout.style.transform = `translateY(${y}%)`;
